...
/// Marker identifying an OpenCode command file this tool owns.
const GENERATED_COMMAND_MARKER: &str = "<!-- generated by agentalign from skill:";

/// Expose every canonical skill as an OpenCode slash command.
///
/// OpenCode loads skills and slash commands through two separate globs. Skills
/// are only reachable through the model's `skill` tool, so a synced skill still
/// leaves `/<name>` unresolved until a command file exists. Written to the
/// singular `command/` directory, since the plural one is what
/// `collect_opencode_commands_as_skills` imports back into the canonical store.
fn sync_opencode_commands(home: &Path, canonical_skills: &[String]) -> anyhow::Result<usize> {
    let command_dir = home.join(".config").join("opencode").join("command");
    ...
    for skill_name in canonical_skills {
        let description = skill_description(home, skill_name)
            .unwrap_or_else(|| format!("Run the {} skill", skill_name));
        let body = format!(
            "---\ndescription: \"{}\"\n---\n{} {} -->\n\nInvoke the `{}` skill and follow it exactly. Arguments: $ARGUMENTS\n",
            yaml_quote(&description),
            GENERATED_COMMAND_MARKER,
            skill_name,
            skill_name
        );
        let path = command_dir.join(format!("{}.md", skill_name));
        let current = std::fs::read_to_string(&path).unwrap_or_default();
        if current == body {
            continue;
        }
        if !current.is_empty() && !current.contains(GENERATED_COMMAND_MARKER) {
            // Hand-written command of the same name; the user's file wins.
            continue;
        }
        std::fs::write(&path, body)?;
        eprintln!("  opencode command /{} -> {}", skill_name, skill_name);
        changed += 1;
    }

    // Drop generated commands whose skill is gone.
    let known: std::collections::HashSet<&str> =
        canonical_skills.iter().map(|s| s.as_str()).collect();
    for entry in std::fs::read_dir(&command_dir)?.filter_map(|e| e.ok()) {
        let path = entry.path();
        let Some(stem) = path.file_stem().and_then(|s| s.to_str()) else {
            continue;
        };
        if path.extension().and_then(|e| e.to_str()) != Some("md") || known.contains(stem) {
            continue;
        }
        let Ok(content) = std::fs::read_to_string(&path) else {
            continue;
        };
        if content.contains(GENERATED_COMMAND_MARKER) && std::fs::remove_file(&path).is_ok() {
            eprintln!("  opencode command /{} -> pruned (skill gone)", stem);
            changed += 1;
        }
    }
...
///
/// Skill descriptions carry colons and quotes, which break an unquoted value.
fn yaml_quote(value: &str) -> String {
    value.replace('\\', "\\\\").replace('"', "\\\"")
}

/// Read a skill's `description:` frontmatter line, collapsed to one line.
fn skill_description(home: &Path, skill_name: &str) -> Option<String> {
    let skill_md = canonical_skills_dir(home)
        .join(skill_name)
        .join("SKILL.md");
...
pub fn heal_all(home: &Path) -> anyhow::Result<usize> {
    ...
    match sync_opencode_commands(home, &canonical_skills) {
        Ok(changed) => fixed += changed,
        Err(e) => eprintln!("  warning: failed to sync opencode commands: {}", e),
    }
...
mod tests {
    ...
    #[test]
    fn test_command_description_is_quoted_and_escaped() {
        assert_eq!(yaml_quote(r#"Use when: a "thing" breaks"#), r#"Use when: a \"thing\" breaks"#);
    }

    #[test]
    fn test_opencode_commands_generated_and_pruned() {
        let (_tmp, home) = setup();
        let canonical = canonical_skills_dir(&home);
        fs::create_dir_all(canonical.join("research-report")).unwrap();
        fs::write(
            canonical.join("research-report").join("SKILL.md"),
            "---\nname: research-report\ndescription: Brief a reviewer\n---\n\nbody",
        )
        .unwrap();
        let command_dir = home.join(".config").join("opencode").join("command");
        fs::create_dir_all(&command_dir).unwrap();
        fs::write(command_dir.join("stale.md"), format!("{} stale -->\n", GENERATED_COMMAND_MARKER)).unwrap();
        fs::write(command_dir.join("mine.md"), "hand written").unwrap();

        heal_all(&home).unwrap();

        let generated = fs::read_to_string(command_dir.join("research-report.md")).unwrap();
        assert!(generated.contains("description: \"Brief a reviewer\""));
        assert!(generated.contains("Invoke the `research-report` skill"));
        assert!(!command_dir.join("stale.md").exists(), "generated command for a gone skill kept");
        assert_eq!(
            fs::read_to_string(command_dir.join("mine.md")).unwrap(),
            "hand written",
            "hand-written command must not be touched"
        );
    }
